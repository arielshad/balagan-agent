...
const TerminalLine: React.FC<{text: string; color?: string; delay: number}> = ({text, color = theme.text, delay}) => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();
      ...
      <Sequence from={90} durationInFrames={210}>
        <AbsoluteFill style={{padding: 60, display: 'flex', flexDirection: 'column', justifyContent: 'center'}}>
          <div style={{background: theme.codeBg, border: `1px solid ${theme.border}`, borderRadius: 12, padding: 36}}>
            ...
            <TerminalLine text="$ pip install balagan-agent" color={theme.textMuted} delay={0} />
            <TerminalLine text="Successfully installed balagan-agent-0.4.0" color={theme.green} delay={40} />
            <div style={{height: 16}} />
            <TerminalLine text="$ balaganagent demo --chaos-level 0.5" color={theme.textMuted} delay={70} />
            <div style={{height: 8}} />
            <TerminalLine text="⚡ Running chaos experiment..." color={theme.accent} delay={100} />
            <TerminalLine text="  Injecting: tool_failure ×4, latency ×3" color={theme.text} delay={120} />
            <TerminalLine text="  Injecting: hallucination ×2, budget ×1" color={theme.text} delay={135} />
            <div style={{height: 8}} />
            <TerminalLine text="✓ 8/10 runs passed (80% reliability)" color={theme.green} delay={160} />
            <TerminalLine text="✓ Mean Time to Recovery: 2.3s" color={theme.green} delay={175} />
